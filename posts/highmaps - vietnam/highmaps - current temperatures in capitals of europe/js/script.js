...
function getTemp(point, countries, capitals) {
    $.ajax({
        url: 'https://api.met.no/weatherapi/locationforecast/1.9/?lat=' + point[1] + '&lon=' + point[2],
        complete: function (data) {
            const colorAxis = countries.chart.colorAxis[0];

            // Parse received data
            const xml = $.parseXML(data.responseText, 'text/xml');
            const temp = $(xml).find('#TTT').attr('value');
            const country = {
                'hc-key': point[0],
