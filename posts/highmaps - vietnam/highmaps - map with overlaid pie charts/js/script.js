...
Highcharts.seriesType('mappie', 'pie', {
    ...
    getCenter: function () {
        ...
        const slicingRoom = 2 * (options.slicedOffset || 0);
        if (!options.center) {
            options.center = [null, null]; // Do the default here instead
        }
        // Handle lat/lon support
        if (options.center.lat !== undefined) {
            const point = chart.fromLatLonToPoint(options.center);
            options.center = [
    ...
    item.setVisible = function () {
        ...
        Highcharts.each(chart.series[0].points, function (point) {
            if (chart.colorAxis[0].dataClasses[point.dataClass].name === legendItem.name) {
                ...
                const connector = Highcharts.find(chart.series[2].points, function (item) {
                    return item.name === point.id;
                });
                if (connector) {
...
Highcharts.each(chart.series[0].points, function (state) {
    ...
    const centerLat = parseFloat(state.properties.latitude);
    const centerLon = parseFloat(state.properties.longitude);
    ...
    chart.addSeries({
        ...
        sizeFormatter: function () {
            ...
            const zoomFactor = (yAxis.dataMax - yAxis.dataMin)
          / (yAxis.max - yAxis.min);
            return Math.max(
    ...
    if (pieOffset.drawConnector !== false) {
        ...
        const offsetPoint = chart.fromLatLonToPoint({
            lat: centerLat + (pieOffset.lat || 0),
            lon: centerLon + (pieOffset.lon || 0)
        });
