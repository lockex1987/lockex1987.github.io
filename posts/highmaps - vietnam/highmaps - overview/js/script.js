...
$('#mapDropdown').change(function () {
    ...
    const mapDesc = $selectedItem.text();
    const mapKey = this.value.slice(0, -3);
    const svgPath = baseMapPath + mapKey + '.svg';
    const geojsonPath = baseMapPath + mapKey + '.geo.json';
    const javascriptPath = baseMapPath + this.value;
    const isHeader = $selectedItem.hasClass('option-header');
