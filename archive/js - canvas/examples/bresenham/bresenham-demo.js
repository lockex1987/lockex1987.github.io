...
function draw_grid(ctx) {
	...
	ctx.strokeStyle = "rgba(204, 204, 204, 0.75)";
	ctx.beginPath();
	for (var x = 0; x < x_size; x += pixel_size) {
		ctx.moveTo(x, 0);
		ctx.lineTo(x, y_size);
	}
	for (var y = 0; y < y_size; y += pixel_size) {
		ctx.moveTo(0, y);
		ctx.lineTo(x_size, y);
	}
	ctx.stroke();
...
function update_circle_view(ctx) {
	draw_grid(ctx);
...
function update_ellipse_view(ctx) {
	draw_grid(ctx);
